  ...
  return (
    <div className="relative w-44">
      <button
        ...
        className="flex justify-between w-full p-3 pr-8 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-0 focus:border-black text-sm text-center items-center"
      >
        {currentCategory ? (
          <>
        ...
        <svg
          className="absolute right-3 w-2.5 h-2.5"
          aria-hidden="true"
          ...
          <div className="absolute right-0 z-20 bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700">
            <ul className="py-2 text-sm text-gray-700 dark:text-gray-200">
              {categories.map((item: Category) => (
                <li
                  key={item.id}
                  className="flex justify-between items-center py-2 px-4 space-x-4"
